...
fun GroupFormScreen(
        ...
        if (uiState.isLoading) {
            ...
            Column(
                ...
                Row(verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                    ...
                    TextButton(onClick = viewModel::onAddMember) { Text("Add") }
                }
    ...
    memberPendingDelete?.let { member ->
        AlertDialog(
            ...
            dismissButton = {
                TextButton(onClick = { memberPendingDelete = null }) { Text("Cancel") }
            }
        )
    }
    ...
    uiState.memberActionError?.let { error ->
        AlertDialog(
            ...
            text = { Text(error) },
            confirmButton = {
                TextButton(onClick = viewModel::dismissMemberActionError) { Text("OK") }
            }
        )
    }
}
